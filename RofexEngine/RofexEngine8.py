...
import logging
import texttable
#from RofexEngine.armarMensajes import armarMensajes
...
class rofexEngine(fix.Application):
    ...
    def fromApp(self, message, sessionID):
        ...
        if tag35 == 'B':  # News
            ...
            logfix.warning("Securities List: allSecurities{} <-- fromApp >> (%s) " % msg)
    ...
    def run(self,ticker):
        ...
        #msgToRofex = armarMensajes(self.usrID, self.targetCompID)
    ...
    def goRobot(self, ticker):
        # Overridable Method

        logfix.info("goRobot: >> (%s)" % self.sessionID)
        self.session_off = False

        self.suscribeMD(ticker)
